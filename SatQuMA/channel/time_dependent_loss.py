...
def get_losses_input_file(file_to_read, loss_params,f_atm,tPrint,outpath):
    """
    Returns the loss array by either generating data or reading from file.
    ...
    if loss_params['tReadLoss']:
        # File containing loss data (for given theta_max value below)
        loss_file = loss_params['loss_file'].format(file_to_read)
        ...
        # if tPrint:
        #     print('Generating losses for theta_max = {} deg'.format(np.degrees(file_to_read)))
        #     print('-'*60)
        zenith_time_dataframe = pd.read_csv(file_to_read)
        try:
            zenith_time_dataframe = zenith_time_dataframe[["currentTime", "elevationAngle", "range"]]
            r = "range"
        except:
            zenith_time_dataframe = zenith_time_dataframe[["currentTime", "elevationAngle", "Range"]]
            r = "Range"
        # Calculate the angle between the orbital plane and the zenith plane
        vals = np.empty((len(zenith_time_dataframe.index), 7))  # Initisalise array to store values
        count = 0  # Initialise time-slot counter
        zenith_time_dataframe["elevationAngle"] = zenith_time_dataframe["elevationAngle"].apply(lambda x: x * np.pi / 180)
        time_step = zenith_time_dataframe["currentTime"].loc[1]- zenith_time_dataframe["currentTime"].loc[0]
        for t in range(len(zenith_time_dataframe.index)):
            elev = zenith_time_dataframe["elevationAngle"].loc[t]

            # Calculate abs&scat for current system by interpolating data
            eta_atm = f_atm(elev)
            L = zenith_time_dataframe[r].loc[t]
            k = 2 * np.pi / (loss_params['wvl'] * 10 **-9)
            aT =loss_params['aT']
            aR = loss_params['aR']
            w0 = loss_params['w0']
            eta_diff = diffract(aT, aR, L, k, w0)
            ...
            # Calculate total efficiency of current system
            eta_tot = eta_atm * eta_diff * loss_params['eta_int']

            # Store data
            vals[count, 0] = zenith_time_dataframe["currentTime"].loc[t]  # Orbital time of current system (s)
            vals[count, 1] = elev  # Elevation angle (degs)
            vals[count, 2] = eta_tot  # Total efficiency for current system
            vals[count, 3] = eta_diff  # Diffraction efficiency for current system
            vals[count, 4] = eta_atm  # Atmospheric transmissivity for current system
            vals[count, 5] = loss_params['eta_int']  # Intrinsic system losses
            vals[count, 6] = L  # Range (m)
            count += 1
        # Header for loss files
        header = 'Time (s),Elevation (rad),eta_tot,eta_diff,eta_atm,eta_sys,Distance (m)'
