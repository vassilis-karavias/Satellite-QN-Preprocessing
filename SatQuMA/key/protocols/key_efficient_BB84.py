...
def get_params(args):
    ...
    fEC         = args['fEC']
    NoPass      = args['NoPass']
    segragation = args["segragation"]
    return mu3, ls, dt, time0pos, Pec, QBERI, Pap, FSeff, Npulse, boundFunc, \
            eps_c, eps_s,num_zero, errcorrFunc, fEC, NoPass, segragation
...
def key_length(x, args):
    ...
    mu3, ls, dt, time0pos, Pec, QBERI, Pap, FSeff, Npulse, boundFunc, \
            eps_c, eps_s, num_zero, errcorrFunc, fEC, NoPass, segragation = get_params(args)
    ...
    prev_pos = 0
    l_tot = 0
    for pos, cofactor, time in segragation:
        mu_FSeff = np.outer(mu, FSeff[prev_pos:pos+1])
        if time == "dark":
            Pec = 5 * (10 **-7)
        else:
            Pec = 2 * (10 ** -4)
        ...
        ### In this case we will only need to use fixed nx....
        ...
        # nx_mu_temp = []
        # for m in nx_mu:
        #     nx_mu_temp.append(m * ratio)
        # nx_mu = np.asarray(nx_mu_temp)
        # nz_mu_temp = []
        # for m in nz_mu:
        #     nz_mu_temp.append(m * ratio)
        # nz_mu = np.asarray(nz_mu_temp)
        # nx_temp = np.zeros((len(nx), len(nx[0])))
        # for i in range(len(nx)):
        #     for j in range(len(nx[i])):
        #         nx_temp[i][j] = nx[i][j] * ratio
        # nx = nx_temp
        # nz_temp = np.zeros((len(nz), len(nz[0])))
        # for i in range(len(nz)):
        #     for j in range(len(nz[i])):
        #         nz_temp[i][j] = nz[i][j] * ratio
        # nz = nz_temp
        ...
        prev_pos = pos + 1
        l_tot += l * cofactor
    return l_tot, QBERx, phi_x, nX, nZ, mXtot, lambdaEC, sx0, sx1, vz1, sz1, mpn
