...
from SatQuMA.key.protocols.init_efficient_BB84 import (x0_rand, x0_init, check_constraints)
from SatQuMA.key.protocols.key_efficient_BB84 import (set_params, key_length, key_length_inv)
from SatQuMA.channel.time_dependent_loss import (get_losses, get_losses_input_file)
from SatQuMA.output.outputs import (getOptData, writeDataCSV, write_data, writeMultiData,
                            get_timings, format_time)
import satellite_rate_calculator as satellite_rate_calculator
...
def SKL_opt_loop_loss_and_time(count,ci,ni,theta_max,Pec,QBERI,ls_range,
                               ...
                               optdata,sys_params,sysLoss, segragation):
    """
...
def SKL_sys_loop(count,ci,ni,x,x0i,xb,theta_max,dt_range,main_params,opt_params,
            ...
            if tOptimise:
                ...
                fulldata, count = \
                    SKL_loop_loss_and_time(count,ci,ni,theta_max,Pec,QBERI,
                                           ...
                                           main_params['out']['tPrint'],
                                           fulldata,sys_params,sysLoss, segragation)
    ...
    for i in range(len(fulldata)):
        if fulldata[i][5] > max_rate:
            max_rate = fulldata[i][5]

    return multidata, count, max_rate
...
def SKL_main_loop(main_params,adv_params,x,x0i,xb,ci,ni,f_atm,bounds,cons,
                  options,header,opt_head,fulldata,optdata,multidata, segragation):
    """
    ...
    for theta_max in main_params['iter']['theta_max']:
        ...
        multidata, count, max_rate = \
            SKL_sys_loop(count,ci,ni,x,x0i,xb,theta_max,dt_range,main_params,
                         ...
                         sys_params,sysLoss, segragation)
...
def SKL_main_loop_2(main_params, adv_params, x, x0i, xb, ci, ni, f_atm, bounds, cons,
                  options, header, opt_head, fulldata, optdata, multidata, satellite_trajectory_file, allocation_method, ground_station, file_path = None, satellite_sites= None, satellite_RAAN = None, network_structure = None, method_for_network_segragation = "ConnectionBased", distance = None, segragations = None, n =1):
    """
    Calculate the SKL using either optimised or specified protocol parameters
    for iterated values of:
        theta_max : Maximum elevation angle (rad)
    ...
    allocation_method: string
        Method of satellite allocation: choice from ["None", "ClashFixedTime", "ClashFullBypass", "ClashFullBypassTransmissionModulated", "ClashFullBypassKeyModulated"]
        If last one is selected a valid Network_Structure class must be passed in
    file_path: string
        Needed. The general name of the files to find the satellite paths from:
        satellite files will be of the form file_path + "_" + satellite_site + "_" + str(satellite_RAAN) + ".csv"
    satellite_sites: string, array-like
        Needed. The array of names of the satellite_sites in consideration:
        satellite files will be of the form file_path + "_" + satellite_site + "_" + str(satellite_RAAN) + ".csv"
        satellite_sites = [satellite_site]
    satellite_RAAN: int
        Needed. The satellite RAAN currently considered:
        satellite files will be of the form file_path + "_" + satellite_site + "_" + str(satellite_RAAN) + ".csv"
    network_structure: Network_Structure
        Only needed if "ClashFullBypassTransmissionModulated" or "ClashFullBypassKeyModulated" is selected as allocation_method
    method_for_network_segragation: string
        Only needed if "ClashFullBypassTransmissionModulated" or "ClashFullBypassKeyModulated" is selected as allocation_method. Selects method for
        how to split the underlying core nodes in the network into possible ground stations
        Possible values from "ConnectionBased", "DistanceBased", "SingleConnection"
    distance: float
        (in km). Only needed if "ClashFullBypassTransmissionModulated" or "ClashFullBypassKeyModulated" is selected as allocation_method and "DistanceBased" is
        used for method_for_network_segragation
    Returns
    -------
    None.
    ...
    loss_data = get_losses_input_file(satellite_trajectory_file, main_params['loss'], f_atm,
                               main_params['out']['tPrint'],
                               main_params['out']['out_path'])

    FSeff = loss_data[:, 2]
    if allocation_method == "None":
        segragation = satellite_rate_calculator.no_clash_method_rates_segragation(loss_data, file_path, satellite_sites, satellite_RAAN, ground_station=ground_station)
    if allocation_method == "ClashFixedTime":
        segragation = satellite_rate_calculator.simple_clash_method_rates_segragation(loss_data, file_path, satellite_sites, satellite_RAAN)
    if allocation_method == "ClashFullBypass":
        segragation = satellite_rate_calculator.simple_clash_method_rates_2_segragation(loss_data, file_path, satellite_sites, satellite_RAAN, n= n, ground_station=ground_station)
    if allocation_method == "ClashFullBypassTransmissionModulated" and segragations == None:
        network_structure.calculate_physical_distance_between_core_sites_and_stations()
        if method_for_network_segragation == "ConnectionBased":
            network_structure.select_available_sites_based_on_connectivity()
        elif method_for_network_segragation == "DistanceBased":
            network_structure.select_available_sites_based_on_distance(distance = distance)
        elif method_for_network_segragation == "SingleConnection":
            network_structure.select_single_available_site()
        segragations = network_structure.calculate_satellite_rate_based_on_expected_transmission_requirements_segragation(file_path, satellite_sites, satellite_RAAN)
        satellite = satellite_trajectory_file.split("_")[2]
        segragation = segragations[satellite]
    elif allocation_method == "ClashFullBypassTransmissionModulated":
        satellite = satellite_trajectory_file.split("_")[2]
        segragation = segragations[satellite]
    dt_max = len(FSeff)  # Maximum time window half-widt
    theta_max = 0
    dt_range = np.asarray(main_params['iter']['dt'])  # Reset initial loop values
    dt_range[dt_range < 0] = 0  # All values must be positive
    dt_range[dt_range > dt_max] = dt_max  # Max cannot exceed No. of time-slots
    ...
    args_fixed = args_fixed_list(main_params['fixed'], adv_params['calc'],
                                 FSeff, time0pos)
    time0elev = loss_data[time0pos, 1]
    max_elev = np.degrees(time0elev)
    sys_params = sys_param_list(main_params['fixed'], max_elev)
    sysLoss = -10 * (np.math.log10(FSeff[time0pos]) +
                     np.math.log10(main_params['fixed']['eta']))

    if allocation_method == "ClashFullBypassKeyModulated" and segragations == None:
        network_structure.calculate_physical_distance_between_core_sites_and_stations()
        if method_for_network_segragation == "ConnectionBased":
            network_structure.select_available_sites_based_on_connectivity()
        elif method_for_network_segragation == "DistanceBased":
            ...
            network_structure.select_single_available_site()
        satellite_paths = satellite_trajectory_file.split("_")
        segragations = network_structure.calculate_satellite_rate_based_on_key_rate_segragation(file_path, satellite_sites, satellite_RAAN,
                                                               count, ci, ni, x, x0i, xb, theta_max, dt_range,
                                                               main_params, adv_params['opt'],
                                                               args_fixed, bounds, cons, options, header, opt_head,
                                                               fulldata, optdata,
                                                               multidata, sys_params, sysLoss, satellite_trajectory_file=satellite_paths[0]+"_"+ satellite_paths[1], f_atm = f_atm)
        satellite = satellite_trajectory_file.split("_")[2]
        segragation = segragations[satellite]
    elif allocation_method == "ClashFullBypassKeyModulated":
        satellite = satellite_trajectory_file.split("_")[2]
        segragation = segragations[satellite]
    multidata, count, max_rate = \
        SKL_sys_loop(count, ci, ni, x, x0i, xb, theta_max, dt_range, main_params,
